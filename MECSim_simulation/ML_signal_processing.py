...
def functionholder(var):
    print("NOT USED IN THE MECWRITTER")
    funcvar_holder = []
...
def Iterative_MECSim_Curr(*args, **kwargs):  # (data, var, val_in, spaces, DCAC_method, Exp_data,harm_weights):
    ...
    #val_in = val_in[0:-1]
    #Ncore = DCAC_method[2]
    # required to check if data changes
    oldExptime = outputsetterDT(data)
    ...
    if funcvar[0][0] != 0:
        funclist = listin_func(val_in, funcvar, funcvar_holder,data,var)
        data = funcvar2data(funclist, spaces, data)
    else:
        pass
    ...
    # this is for varible length of time adds a set amount of time accuracy if larger
    newExptime = outputsetterDT(data)
    # checks for varible time
    if newExptime > oldExptime:
        dpoints = int(np.log2(newExptime/oldExptime)) + int(float(data.iloc[6][0]))
        data.iloc[6][0] = format_e(dpoints)
...
def listin_func(val_in, funcvar, funcvar_holder,data,var):
    funclist = []
    i = 0
    while i != funcvar[0][0]:
...
def harm_gen_fft(fft_res, time, AC_freq, bandwidth, spaces):
