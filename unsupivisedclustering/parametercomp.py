...
dname = ["postgres", "Quantum2", "localhost", 5432, "timeseriestest"]
...
for l in ptot:
    ...
    if len(l) > 100:
        ...
        axs[1].plot(model.cluster_centers_[i].ravel())
        x.append(len(l))
    i += 1
...
for l in ptot:
    ...
    if len(l) > 100:
        ...
        axs[2].plot(model.cluster_centers_[i].ravel())
        x.append(len(l[:,0]))
    i += 1
...
for l in ptot:
    ...
    if len(l) > 100:
        ...
        plotnum.append(l[np.random.randint(l.shape[0]),2]) # gets a random thing to plot
        #axs[1].set_xscale("log")
        axs[2].plot(model.cluster_centers_[i].ravel(),color=colour[ii])
        x.append(len(l[:,0]))
        ii += 1
    i += 1
